...
const BookItemFrame = memo(function BookItemFrame({ id, title, book_description, book_img }: Book) {
    ...
    return (
        <Link to={`/book/${id}`} className="flex flex-col h-100 rounded-lg shadow-md overflow-hidden hover:shadow-lg transition">
            ...
            <div className="flex flex-col gap-2 bg-white p-4 flex-1 min-h-0 overflow-hidden">
                ...
                <p className="text-gray-600 text-sm text-justify line-clamp-7 overflow-hidden">{book_description}</p>
            </div>
        </Link>
    );
});
